...
**Current note:** Added portable PIT acceptance review bundle for GPT Pro under `docs/pit_acceptance_review/` plus focused GitHub Action `.github/workflows/pit-acceptance-review.yml`. Bundle includes compact acceptance summary, Gate 1-11 summaries, publish readiness JSONs, complete-panel validation JSONs, reconciliation audit counts, artifact SHA-256 hashes, and exact run commands for `2022Q1` and `2026Q1` full acceptance roots. Purpose: let reviewers inspect evidence from GitHub without local `/Users/...` access and get a narrow GitHub check for the repair path. Verification: focused tests `18 passed`; portable evidence validator passed; `git diff --check` passed. Last updated 2026-05-18T14:04:20-04:00.
...
**Current note:** Completed GPT Pro review packet requirements for PIT master repair. Added emitted price-basis fields, ticker/CIK PIT mapping fields, and schema coverage; regenerated smoke outputs with these fields. Latest smoke roots: `tradingagents/research/fundamental/runs/2022-03-31/2022Q1/pit_repair_smoke_v3` and `tradingagents/research/fundamental/runs/2026-03-31/2026Q1/pit_repair_smoke_v3`. Both smoke runs are `scout-smoke`, final `false`, Gates `1-7` passed, Gate `8` skipped by design, Gate `9` passed, Gate `10` skipped by design. Smoke final score CSVs contain all requested review fields: 2022Q1 has `201` rows / `281` columns; 2026Q1 has `191` rows / `281` columns. Added handoff doc: `tradingagents/research/fundamental/docs/pit_master_repair_gpt_pro_review_handoff.md`. Verification: focused PIT suite `201 passed, 1 skipped`; smoke review-field audit passed; `py_compile` passed; `git diff --check` passed. Last updated 2026-05-18T12:39:02-04:00.

**Current note:** PIT master repair implementation code is committed on branch `pit-master-repair-row-contract` in worktree `/Users/aeternusholdings/.config/superpowers/worktrees/Aeternus/pit-master-repair-row-contract`. Implemented row availability contract, invalid-master quarantine docs, PIT-safe CompanyFacts selector, real trading-session entry dates, PIT financial selection before scoring, panel no-rewrite guard, future-label isolation, selection input logging/gating, LLM source contract, legacy score aliases, missingness preservation, and reconciliation gate. Focused PIT repair suite passed: `197 passed, 1 skipped`. Full repo test suite result: `2405 passed`, `127 failed`, `2 skipped`, `1 xfailed`, `4 xpassed`; failures are broad non-PIT areas such as Aeternus scorer, old analyst import path, CLI wiring, coherence/content/ledger tests. Worktree is clean and `git diff --check` passed. Next plan step: run 2022Q1 and 2026Q1 smoke rebuilds before any full walkforward rebuild. Last updated 2026-05-18T12:01:46-04:00.

**Current note:** Completed broader complete-panel derived-field audit and fixed the same bug class beyond `pre_llm_candidate_flag`. Confirmed all schema columns exist in PIT, then found true derived-field misses: `pre_llm_rank_by_score_quarter`, `entry_score_rank_by_quarter`, `llm_required_derived_flag`, `llm_complete_derived_flag`, `has_post_llm`, `post_llm_any_flag`, plus provenance fields `panel_build_id`, `panel_build_timestamp`, and `feature_schema_path`. Added normalizer derivation and tests. Refreshed PIT master and run-level complete-panel CSV copies. Corrected PIT counts: `pre_llm_rank_by_score_quarter` filled for all `5,526` pre-LLM candidates; `entry_score_rank_by_quarter` filled for all `22,025` rows; `llm_required_derived_flag=1` for `5,526`; `llm_complete_derived_flag=1`, `has_post_llm=1`, and `post_llm_any_flag=1` for `5,525`; duplicates remain `0`. Remaining blank/all-zero fields are legacy, future, unavailable, or no-event fields unless separately promoted. Last updated 2026-05-17T20:53:00-04:00.

**Current note:** Fixed complete-panel derived pre-LLM/aggregate flags. Root cause: `pre_llm_candidate_flag`, `tier_0_to_4_any_flag`, `tier_1_to_4_any_flag`, `hp_any_flag`, and `rm_any_flag` were schema fields but the normalizer only defaulted blank flags to `0`; it did not derive them from Tier/HP/RM source columns. Added derivation in `tradingagents/research/fundamental/src/panel/normalize.py`, regression coverage in `tests/test_fundamental_panel_normalize.py`, and contract wording in `tradingagents/research/fundamental/docs/complete_candidate_panel_contract.md`. Refreshed PIT master plus run complete-panel CSV copies. Fixed PIT counts: `pre_llm_candidate_flag=1` for `5,526` rows, `tier_0_to_4_any_flag=1` for `6,986`, `tier_1_to_4_any_flag=1` for `4,082`, `hp_any_flag=1` for `14,964`, `rm_any_flag=1` for `445`; duplicates remain `0`. Verification: focused panel/daily tests `88 passed`; `git diff --check` passed. Last updated 2026-05-17T13:35:00-04:00.

**Current note:** Completed clean walkforward through `2026Q2` and appended it to PIT. `2026Q2` run root: `tradingagents/research/fundamental/runs/2026-05-15/2026Q2/walkforward_clean_v1`; prior comparison: `2026Q1` clean final scores. Preflight universe `1,374`; ready rows `1,353`; fetch queue `0`; foreign-filer blockers `20`; missing companyfacts `1`. LLM packets `415` total (`333` current, `82` prior recovery), `0` empty evidence, `1` prior-impossible/no-filing ticker (`TRAW`). Four LLM chunks completed `104/104/104/103`; combined file has `415/415` rows with `0` missing and `0` errors. Final run passed Gates `1-11`. Final rows `1,314`; LLM-complete rows `333`; QoQ missing rows `0`; Top15 rows `15`; shadow rows `15`; shadow refill replacements `0`; complete panel validation passed. PIT master appended `1,314` rows and now has `22,025` rows across `2022Q1`-`2026Q2` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Last updated 2026-05-17T13:05:00-04:00.

**Current note:** Completed clean walkforward for `2026Q1` and appended it to PIT. Run root: `tradingagents/research/fundamental/runs/2026-03-31/2026Q1/walkforward_clean_v1`. SEC pass fetched `2,179` docs. LLM packets `492` total (`374` current, `118` prior recovery), `0` empty evidence, prior-impossible/no-filing tickers `CRDO`, `SCL`. Four LLM chunks completed and combined `492/492` with `0` errors. Final run passed Gates `1-11`. Final rows `1,221`; LLM-complete rows `374`; QoQ missing rows `0`; Top15 rows `15`; shadow rows `15`; shadow refill replacements `0`. PIT master appended `1,221` rows and reached `20,711` rows before `2026Q2`. Last updated 2026-05-17T13:00:00-04:00.

**Current note:** Completed clean walkforward for `2025Q4` and appended it to PIT. Run root: `tradingagents/research/fundamental/runs/2025-12-31/2025Q4/walkforward_clean_v1`. SEC pass fetched `3,459` docs. LLM packets `392` total (`325` current, `67` prior recovery), `0` empty evidence, prior-impossible/no-filing tickers `BV`, `CODI`, `GBX`. Four LLM chunks completed and combined `392/392` with `0` errors. Final run passed Gates `1-11`. Final rows `1,233`; LLM-complete rows `325`; QoQ missing rows `0`; Top15 rows `15`; shadow rows `15`; shadow refill replacements `0`. PIT master appended `1,233` rows and reached `19,490` rows before `2026Q1`. Last updated 2026-05-17T12:55:00-04:00.

**Current note:** Completed clean walkforward for `2025Q3` and appended it to PIT. Run root: `tradingagents/research/fundamental/runs/2025-09-30/2025Q3/walkforward_clean_v1`. Previously interrupted LLM chunks were complete; combined file has `431/431` rows with `0` missing/errors. Final run passed Gates `1-11`. Final rows `1,232`; LLM-complete rows `346`; QoQ missing rows `0`; Top15 rows `15`; shadow rows `15`; shadow refill replacements `0`. Prior-impossible/no-filing tickers: `IRTC`, `PACS`. PIT master appended `1,232` rows and reached `18,257` rows before `2025Q4`. Last updated 2026-05-17T12:50:00-04:00.

**Current note:** Storage fix completed. Patched SEC fetcher so `complete_submission` queue items materialize requested documents without storing full SEC complete-submission wrapper `.txt` files. Deleted redundant full wrapper files under `/Users/aeternusholdings/Documents/GitHub/AeternusHoldings/cache/sec/live_sec/complete_submissions`, freeing about `43 GB`. Kept extracted docs, companyfacts, submissions, LLM cache, scores, and PIT. Last updated 2026-05-17T12:45:00-04:00.

**Current note:** Deleted only 2022 raw periodic filing document copies (`10-Q/10-K/10-Q/A/10-K/A`) for `2022Q1`-`2022Q4` from `live_sec/documents`, `sec_docs_html`, and `sec_docs_text`. Deleted `14,562` files and freed `30,118.69 MB`. Cleanup manifest: `eval_results/fundamental/sec_raw_cleanup_2022_periodic_2026-05-17/summary_after_delete.json`. Not deleted: companyfacts, submissions, complete submissions, earnings 8-K primary docs, earnings press-release exhibits, LLM extraction cache, final score/PIT CSVs. Last updated 2026-05-17T10:55:24-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2024Q4`, using `2024Q3` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2024-12-31/2024Q4/walkforward_clean_v1`. Preflight used cached SEC docs for the remaining fetch queue (`1,148` cached existing, `0` fetched), found `359` required LLM packets (`294` current, `65` prior recovery), `0` empty-evidence packets, and `0` prior-impossible/no-filing names. Ran four parallel local LLM chunks, all completed, then combined and validated `359 / 359` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,233`, LLM-complete rows `294`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,233` rows and now has `14,575` rows across `2022Q1`-`2024Q4` with `0` duplicate ticker-quarter rows. Also deleted only 2021Q4 raw periodic filing document copies (`10-Q/10-K/10-Q/A/10-K/A`) from `live_sec/documents`, `sec_docs_html`, and `sec_docs_text`: `3,633` files, `7,186.18 MB` freed. Cleanup manifest: `eval_results/fundamental/sec_raw_cleanup_2021Q4_periodic_2026-05-17/summary_after_delete.json`. Not deleted: companyfacts, submissions, earnings 8-K, press-release exhibits, LLM cache, scores, PIT. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2025Q1`, using `2024Q4` final scores as prior comparison. Last updated 2026-05-17T10:40:29-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2024Q3`, using `2024Q2` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2024-09-30/2024Q3/walkforward_clean_v1`. Preflight fetched `3,491` SEC docs into live cache, found `391` required LLM packets (`309` current, `82` prior recovery), `0` empty-evidence packets, and `0` prior-impossible/no-filing names. Ran four parallel local LLM chunks, all completed, then combined and validated `391 / 391` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,229`, LLM-complete rows `309`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,229` rows and now has `13,342` rows across `2022Q1`-`2024Q3` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2024Q4`, using `2024Q3` final scores as prior comparison. Last updated 2026-05-17T09:46:32-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2024Q2`, using `2024Q1` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2024-06-30/2024Q2/walkforward_clean_v1`. Preflight used cached SEC docs for the remaining fetch queue (`1,147` cached existing, `0` fetched), found `347` required LLM packets (`285` current, `62` prior recovery), `0` empty-evidence packets, and `2` prior-impossible/no-filing names (`BMEA`, `PGY`). Ran four parallel local LLM chunks, all completed, then combined and validated `347 / 347` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,231`, LLM-complete rows `285`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,231` rows and now has `12,113` rows across `2022Q1`-`2024Q2` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2024Q3`, using `2024Q2` final scores as prior comparison. Last updated 2026-05-17T09:02:33-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2024Q1`, using `2023Q4` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2024-03-31/2024Q1/walkforward_clean_v1`. Preflight used cached SEC docs for the remaining fetch queue, found `420` required LLM packets (`324` current, `96` prior recovery), `0` empty-evidence packets, and `3` prior-impossible/no-filing names (`AHR`, `BTSG`, `CORZ`). Ran four parallel local LLM chunks, all completed, then combined and validated `420 / 420` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,220`, LLM-complete rows `324`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,220` rows and now has `10,882` rows across `2022Q1`-`2024Q1` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2024Q2`, using `2024Q1` final scores as prior comparison. Last updated 2026-05-17T08:08:02-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2023Q4`, using `2023Q3` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2023-12-31/2023Q4/walkforward_clean_v1`. Preflight fetched `3,468` SEC docs into live cache, found `488` required LLM packets (`352` current, `136` prior recovery), `0` empty-evidence packets, and `2` prior-impossible/no-filing names (`LXFR`, `PENG`). Ran four parallel local LLM chunks, all completed, then combined and validated `488 / 488` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,224`, LLM-complete rows `352`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,224` rows and now has `9,662` rows across `2022Q1`-`2023Q4` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2024Q1`, using `2023Q4` final scores as prior comparison. Last updated 2026-05-16T23:11:14-04:00.

**Current note:** Completed smoke/preflight and full clean walkforward for `2023Q3`, using `2023Q2` final scores as prior comparison data. Run root: `tradingagents/research/fundamental/runs/2023-09-30/2023Q3/walkforward_clean_v1`. Preflight found `370` required LLM packets (`284` current, `86` prior recovery), `0` empty-evidence packets, and `3` prior-impossible/no-filing names (`MCW`, `SBGI`, `TG`). Ran four parallel local LLM chunks, all completed, then combined and validated `370 / 370` rows with `0` missing sample IDs and `0` missing required fields. Final run passed Gates `1-11`. Counts: final rows `1,219`, LLM-complete rows `283`, QoQ missing rows `0`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. Complete panel validation passed. PIT master appended `1,219` rows and now has `8,438` rows across `2022Q1`-`2023Q3` with `0` duplicate ticker-quarter rows. Focused verification: daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Next quarter: `2023Q4`, using `2023Q3` final scores as prior comparison. Last updated 2026-05-16T21:50:15-04:00.

**Current note:** Completed next clean walkforward quarter `2023Q2` using `2023Q1` final scores as prior comparison. Final run root: `tradingagents/research/fundamental/runs/2023-06-30/2023Q2/walkforward_clean_v2`. Gate 4 fetched/materialized SEC docs and again needed manual `BHF` alias copies for `BHFAO`/`BHFAP` under accession `0001685040-23-000026`. Fixed prior-LLM completeness logic so legitimate `0` flags count as present; this reduced required packets from `559` to `368` (`281` current, `87` prior recovery). Reused the already completed exact LLM rows, filtered to the corrected packet set, and reran final scoring. Final 2023Q2 run passed Gates `1-11`. Counts: final rows `1,208`, post-LLM rows `368`, LLM-complete rows `281`, QoQ missing `0`, Top15 `15`, shadow `15`, shadow refill replacements `0`; complete panel validation passed. PIT master now has `7,219` rows across `2022Q1`-`2023Q2` and `0` duplicate ticker-quarter rows. Next quarter: `2023Q3`, using `2023Q2` final scores as prior comparison. Last updated 2026-05-16T20:32:00-04:00.

**Current note:** Before starting `2023Q3`, added speed/reuse fixes. SEC Gate 4 now materializes alias ticker document filenames from already-cached parent filing docs before/after fetch passes, so preferred-share names like `BHFAO/BHFAP` should not require manual copy. LLM outputs now save to shared cache `/Users/aeternusholdings/Documents/GitHub/AeternusHoldings/cache/sec/llm_extractions/daily_post_llm_cache.csv`; daily LLM service checks this cache before writing/running an LLM job and only sends missing packets. Seeded the cache from existing walkforward completed post-LLM files: `1,987` unique rows. Verification passed: focused daily/coverage/LLM tests `62 passed`; `git diff --check` passed. Last updated 2026-05-16T20:58:00-04:00.

**Current note:** Completed next clean walkforward quarter `2022Q4` using `2022Q3` final scores as prior comparison. Run root: `tradingagents/research/fundamental/runs/2022-12-31/2022Q4/walkforward_clean_v1`. Gate 4 fetched `3,409` SEC docs into live cache because old raw SEC HTML had been deleted and live cache did not yet contain all Q4 docs; `BHFAO`/`BHFAP` needed the known `BHF` alias copy again. Initial LLM need was `324` packets; reused `264` exact saved rows and completed `60` missing rows. QA then fixed two daily-run gaps: partial prior LLM defaults no longer count as complete prior review, and cached `NaN` open prices no longer count as usable prices. With stricter prior rule, total Q4 packet set became `639`; reused `520` exact saved rows and completed `119` missing prior `2022Q3` rows. Final Q4 run passed Gates `1-11`. Counts: final rows `1,206`, LLM-complete rows `322`, QoQ missing `0`, Top15 `15`, shadow `15`, shadow refill replacements `0`; complete panel validation passed. PIT master appended `1,206` rows and now has `4,805` rows across `2022Q1`-`2022Q4`. Focused verification: `13 passed`. Next quarter: `2023Q1`, using `2022Q4` final scores as prior comparison. Last updated 2026-05-16T17:25:00-04:00.

**Current note:** Clean official walkforward now starts at `2022Q1`; `2021Q4` is comparison-only. Completed the `76` missing LLM rows for `2022Q1`, combined them with `151` reused exact ticker-quarter LLM rows, and produced `post_llm_scores_completed_2022Q1.csv` with `227` unique sample IDs. Final clean run: `tradingagents/research/fundamental/runs/2022-03-31/2022Q1/walkforward_clean_v2`. Gates `1-11` passed. Counts: universe `1,251`, final rows `1,197`, price quarantine `6`, score-input quarantine `48`, LLM-complete rows `170`, Top15 rows `15`, shadow rows `15`, shadow refill replacements `0`. `NRDS` is allowed one missing prior QoQ comparison because prior earnings filing is clearly impossible. Complete panel validation passed with `0` errors. PIT master appended at `outputs/fundamental_backtest/pit_master/fundamental_pit_master.csv` with `1,197` rows for `2022Q1`. Code fixes: LLM rows cannot overwrite fresh price/date/base score fields; impossible prior filing cases do not block first official quarter QoQ. Verification: focused finalize tests `8 passed`, py_compile passed. Last updated 2026-05-16T12:47:00-04:00.

**Current note:** Completed next clean walkforward quarter `2022Q2` using `2022Q1` final scores as prior comparison. Run root: `tradingagents/research/fundamental/runs/2022-06-30/2022Q2/walkforward_clean_v1`. Gate 4 initially fetched `3,382` SEC docs and stopped on `BHFAO`/`BHFAP` alias docs; copied matching `BHF` filing docs under preferred-share ticker names and reran. LLM need: `301` packets total (`300` current, `1` prior recovery). Reused `233` exact saved LLM rows, completed only `68` missing `2022Q2` LLM rows, and wrote `post_llm_scores_completed_2022Q2.csv` with `301` unique sample IDs. Gates `1-11` passed. Counts: final rows `1,198`, LLM-complete rows `300`, QoQ missing `0`, Top15 `15`, shadow `15`, shadow refill replacements `0`; complete panel validation passed with `0` errors. PIT master appended `1,198` rows; total PIT rows now `2,395`. Last updated 2026-05-16T14:53:00-04:00.

**Current note:** Completed next clean walkforward quarter `2022Q3` using `2022Q2` final scores as prior comparison. Run root: `tradingagents/research/fundamental/runs/2022-09-30/2022Q3/walkforward_clean_v1`. Gate 4 initially fetched `3,395` SEC docs and stopped on `BHFAO`/`BHFAP` alias docs; copied matching `BHF` filing docs under preferred-share ticker names and reran. LLM need: `268` current-quarter packets. Reused `213` exact saved LLM rows, completed only `55` missing `2022Q3` LLM rows, and wrote `post_llm_scores_completed_2022Q3.csv` with `268` unique sample IDs. Gates `1-11` passed. Counts: final rows `1,204`, LLM-complete rows `268`, QoQ missing `0`, allowed prior-impossible QoQ rows `2`, Top15 `15`, shadow `15`, shadow refill replacements `0`; complete panel validation passed with `0` errors. PIT master appended `1,204` rows; total PIT rows now `3,599`. Last updated 2026-05-16T15:53:00-04:00.

**Current note:** Clean 2021Q4 walkforward restart is using no old combined panel CSV for scoring. Deleted the prior bad combined/PIT CSV outputs, rebuilt `2021Q4` from `master_fundamental_universe_start_2021Q4.json`, and reached Gate 8. User instructed not to run LLM extraction again. Reused prior exact-quarter post-LLM extraction rows into `tradingagents/research/fundamental/runs/2021-12-31/2021Q4/walkforward_clean_v1/reused_post_llm_scores_2021Q4.csv`: `114 / 131` required rows matched, `17` exact `2021Q4` rows still missing (`AXTI`, `BMEA`, `BTU`, `CCC`, `CIFR`, `DCGO`, `EVER`, `LBTYK`, `LFCR`, `LULU`, `MIR`, `MWA`, `NVDA`, `OII`, `PCG`, `ROST`, `VTRS`). Rerun with `--llm-mode post-file` hard-stopped correctly at Gate 8 with no duplicate/unexpected/malformed rows. Missing list saved at `reused_post_llm_missing_2021Q4.csv`; source provenance saved at `reused_post_llm_sources_2021Q4.csv`. Last updated 2026-05-16T11:04:58-04:00.

**Current note:** Refreshed the combined fundamental complete panel at `outputs/fundamental_backtest/full_complete_panel_2021Q4_2026Q2/fundamental_complete_prellm_to_top15_2021Q4_2026Q2.csv` using canonical `2021Q4`-`2026Q1` history plus the validated `2026-05-15` daily final run. New counts: `23,619` rows, `330` columns, `1,365` unique tickers, `19` quarters, `2026Q2` rows `1,316`. Review fix preserved historical Top15/shadow flags while updating current-quarter selections: Top15 selected `206`, shadow selected `272`; validation passed with `0` duplicate ticker-quarter rows and `0` errors. Manifest financial blanks now match the CSV: revenue `5`, assets `2`, other core financial fields `0`. Current Top15 rows keep selector rank/treatment/operating metadata, and source-file provenance now points to repo-local `source_artifacts/` instead of machine-local worktree paths. Last updated 2026-05-16T12:58:00-04:00.
...
**Current note:** Rebuilt the final `2026-05-14` dealflow ticker handoff after same-date X-feed finalized. Inputs: non-X scout summary `108` unique, X-feed merged `35` unique, active manual watchlist `16` unique. Final union count is `144` tickers. Source deltas: X added `25` names not in non-X scouts; manual watchlist added `11` names not already in non-X scouts or X. Updated `eval_results/deal_flow/2026-05-14/final_dealflow_tickers.{json,txt}`, `latest_final_dealflow_tickers.{json,txt}`, and refreshed `total_unique_tickers.{json,txt,md}`. Verification: no `score`, `rank`, `deal_flow_score`, `core_score`, or `triage_score` keys in final handoff or total artifact. Last updated 2026-05-14T12:41:49-04:00.

**Current note:** Ran extended adversarial/architecture review loop on `docs/superpowers/plans/2026-05-14-fundamental-daily-main-orchestration.md` after user asked twice for confidence. Revised plan now documents `25` current pipeline gaps and `7` review-loop findings, with `11` sequential implementation tasks. New loopholes closed in the plan: LLM recovery must include Gate 7 quarantine rows before packet build; SEC direct lookup names official SEC sources and must be injectable/mocked in tests; companyfacts split roots must be handled; rejected new dealflow tickers must not affect broad denominator; master tickers missing price must reconcile through explicit quarantine; queued/subagent LLM mode is pending, not final; Yahoo fetch must batch and save partial successes; missing daily handoff hard-stops broad-final unless explicitly allowed; date/quarter mismatch has a guard; SEC/Yahoo ticker aliases must be recorded; daily publish must enable selector coverage gating; default master source is immutable 2021Q4 start file plus append-only `master_fundamental_universe_additions.jsonl`, not the start file alone forever. Also updated `tradingagents/research/fundamental/docs/daily_universe_llm_funnel_contract.md` and `daily_run_gate_sequence.md`. Last updated 2026-05-14T11:58:00-04:00.

**Current note:** Task 2 complete in feature worktree. Added daily input guardrails in `tradingagents/research/fundamental/src/cli/commands.py`, `tradingagents/research/fundamental/src/daily_run/models.py`, and `tradingagents/research/fundamental/src/daily_run/orchestrator.py`. CLI now exposes `--allow-missing-handoff` and `--allow-date-quarter-mismatch`; broad-final hard-stops on missing handoff or date/quarter mismatch unless explicitly allowed; diagnostic-only reports `handoff_missing` plainly; scout-smoke requires a handoff unless missing is explicitly allowed. Updated tests in `tests/test_cli_fundamental_run_today.py` and `tests/test_fundamental_daily_orchestrator_contract.py`. Verification passed: `python3 -m pytest tests/test_cli_fundamental_run_today.py tests/test_fundamental_daily_orchestrator_contract.py -q` -> `19 passed`. Last updated 2026-05-14T12:30:34-04:00.

**Current note:** Saved the corrected plain daily fundamental run steps into `tradingagents/research/fundamental/docs/daily_run_gate_sequence.md` and created the implementation plan at `docs/superpowers/plans/2026-05-14-fundamental-daily-main-orchestration.md`. The plan explicitly says not to create a new pipeline; `fundamental-run-today` remains the only daily main path. Current gaps documented: wrong default master source, no automatic new-dealflow identity fallback, SEC-map review-list mode is not official main path, main price path lacks cache writeback, LLM evidence recovery is not inside Gate 8, raw document loading is too narrow, and status files are split/incomplete. Last updated 2026-05-14T11:24:00-04:00.

**Current note:** Created the new master fundamental universe start file from 2021Q4 SEC evidence. File: `tradingagents/research/fundamental/data/master_fundamental_universe_start_2021Q4.json`. Rule: unique tickers with quarter-level 2021Q4 SEC evidence in local historical fundamental files; price-only and companyfacts-only ticker evidence do not create rows. Count: `1,251` tickers and all `1,251` now have CIK/company title. Local `sec_company_tickers.json` missed `CFLT`, `EXAS`, and `TGNA`, so identity was filled from the existing complete panel: CFLT CIK `1699838` `CONFLUENT, INC.`, EXAS CIK `1124140` `EXACT SCIENCES CORP`, TGNA CIK `39899` `TEGNA INC.`. Default rule: if SEC ticker map misses, check trusted panel/SEC facts/SEC directly before marking unresolved. Supporting inventory artifacts: `eval_results/fundamental/master_universe_start_2021Q4_inventory/`. Updated `tradingagents/research/fundamental/docs/fundamental_stock_list_origin.md`. Last updated 2026-05-14T11:06:00-04:00.

**Current note:** Checked SEC/cache for historical LLM evidence-recovery rows. The backfill matcher initially missed cached earnings files with short quarter tokens like `4q23` and names like `pressrelease`, `earningsslides`, `prearnings`, and `earnings991`; matcher was expanded and tests now pass. Current backfill result after rerun: `1,102` LLM-required rows need completion, `296` packets are ready to run from cached text evidence, and `806` rows still need evidence recovery first. Of the `806`, `31` have possible cached evidence not packetized yet (`20` in `sec_docs_text`, `11` in `live_sec/documents`), and `775` have ticker cache docs but no quarter-matched earnings/8-K evidence in checked roots. Artifacts: `eval_results/fundamental/llm_backfill_2021Q4_2026Q2/sec_cache_evidence_recovery_audit_after_cache_fix.*`. Verification: `tests/test_fundamental_llm_backfill.py -q` -> `16 passed, 2 warnings`; py_compile passed. Last updated 2026-05-14T08:50:00-04:00.

**Current note:** Updated `AGENTS.md` communication guidance for LLM coverage wording after user correction. Default wording must distinguish states: "LLM-required rows need completion" means the framework says those rows need LLM review; "packets ready to run" means source earnings/8-K text was found and a runnable packet exists; "rows need evidence recovery first" means no usable earnings/8-K text was found yet. Never say "missing LLM packets" when the real issue is pending LLM completion or missing evidence. For current historical backfill after SEC/cache matcher update: `1,102` LLM-required rows need completion, `296` packets are ready to run, and `806` rows need evidence recovery first. Last updated 2026-05-14T08:50:00-04:00.

**Current note:** Implemented historical LLM backfill preparation and trigger support for the fundamental complete panel. New module `tradingagents/research/fundamental/src/panel/llm_backfill.py` scans a panel CSV over editable inclusive quarters, identifies LLM-required-but-not-complete rows using derived flags with fallback fields, writes missing rows CSV, ticker TXT, summary JSON, missing manifest JSONL, runnable `historical_llm_backfill_evidence_packets.jsonl`, and evidence-missing CSV. Evidence build uses panel fields (`earnings_8k_accession`, primary/exhibit docs, `text_path`) and a conservative ticker+quarter filename fallback under `--sec-text-root` for names like `q1x2024`, while rejecting generic 10-Q/10-K files including linked `text_path` files. Runnable evidence packets use a narrow explicit allowlist for identity, pre-LLM score/bucket, tier buckets, and HP/RM pre-LLM/tier-derived extensions only; theme/narrative/filing-theme conclusions, post-LLM/final/backtest/monitor/decision/candidate-state, price, QoQ, outcome, diagnostic, selection/top15/shadow, forward-return, and source/provenance fields stay out of LLM input. CLI command `fundamental-llm-backfill` supports `--llm-mode prepare|subagent|in-session|external|post-file`, model/reasoning/batch/output controls, strict historical post-file validation via `validate_llm_result`, JSON decoding for runner-shaped list fields, ticker/quarter identity mismatch checks, deterministic-field mismatch checks including theme acceleration fields, and non-prepare hard stop when no runnable evidence packets exist. Updated docs with command usage, trigger modes, manifest-vs-packet wording, and sanitization note. Verification: red tests covered missing evidence/mode support, outcome leakage, final-stage leakage, invalid post-file values, linked 10-Q text_path, theme/LLM-ish leakage, deterministic mismatch, runner-shaped JSON list fields, ticker/quarter mismatch, and theme acceleration mismatch; final `python3 -m pytest tests/test_fundamental_llm_backfill.py -q` -> `14 passed, 2 warnings`; `python3 -m py_compile tradingagents/research/fundamental/src/panel/llm_backfill.py tradingagents/research/fundamental/src/cli/commands.py` passed. Last updated 2026-05-14T08:30:57-04:00.

**Current note:** Ran the non-X dealflow scouts for `2026-05-14` with X manually disabled in the source handoff. Preflight results: technical universe refresh `515` unique tickers from `SPY/QQQ/DOW`, technical signal sync `516` tickers with `515` inserted rows and `27` buy-zone states, commodity scan `0`, DoD scan `0` with API 400 warnings for categories `621` and `237`, and Yahoo missing-price warning for `CTRA`. Final no-X scout summary: `108` unique tickers, `136` total scout mentions, `x_manual_feed=0`, `breakout_scan=61`, `thirteenf_watchlist=68`, `insider_cluster=2`, `fma_recall=5`, all other scouts `0`. Verified `final_dealflow_tickers.json` has `108` tickers and no `score`, `rank`, `deal_flow_score`, `core_score`, or `triage_score` fields; metadata keys are only `scouts`. Artifacts: `eval_results/deal_flow/2026-05-14/scout_ticker_summary.{md,json}` and `eval_results/deal_flow/2026-05-14/final_dealflow_tickers.{json,txt}`. Last updated 2026-05-14T07:24:31-04:00.

**Current note:** Documented the permanent historical fundamental coverage findings at `tradingagents/research/fundamental/docs/historical_coverage_findings_2021Q4_2026Q2.md` and linked it from the fundamental docs README. Key facts: complete panel has `23,602` rows including `1,299` for `2026Q2`; unique tickers across the full period are `1,319`; `1,023` tickers have all `19` quarters in the complete panel; all-root evidence inventory has `6,852` tickers with some evidence, `1,413` with quarter-level evidence, `23,806` ticker-quarter evidence rows, and `1,035` tickers with all `19` quarters represented. Recalculation caveat: current files can replay/review existing scored panel, but not cleanly recalculate every LLM-required row from raw inputs because `1,102` ticker-quarter rows are marked LLM-required but not LLM-complete. This does not mean all tickers need LLM; LLM applies only to rows marked LLM-required. For `2026Q2`, `256` rows are LLM-required, `205` are required-and-complete, `51` are required-and-pending, and `4` additional rows are LLM-complete but not marked required by the derived flag, giving `209` total rows with `llm_status = complete`. Last updated 2026-05-14T07:17:41-04:00.

**Current note:** Updated the SEC evidence count to check all user-listed roots, not only the current `live_sec` cache. New reusable inventory script: `tradingagents/research/fundamental/src/sec_pipeline/evidence_inventory.py`. Output root: `eval_results/fundamental/sec_evidence_inventory_2021Q4_2026Q2/`. Result for `2021Q4` through `2026Q2`: `6,852` tickers have some evidence anywhere, `1,413` tickers have quarter-level evidence, `23,806` ticker-quarter evidence rows exist, and `1,035` tickers have all `19` quarters represented by at least one quarter-level source. Important caveat: this is broad evidence coverage across raw SEC files, company facts, LLM outputs, old score CSVs, complete panel rows, and price cache; it is not the same as fully ready for final scoring. Verification: `python3 -m compileall -q tradingagents/research/fundamental/src/sec_pipeline/evidence_inventory.py` passed. Last updated 2026-05-14T06:41:29-04:00.

**Current note:** Implemented the plain SEC-to-review-list flow for the fundamental framework. New module `tradingagents/research/fundamental/src/daily_run/review_list_filter.py` starts from SEC company tickers, requires earnings 8-K, press-release exhibit, periodic filing, core company data, raw Yahoo OHLCV, raw close `>= 2`, and `ADV60 >= 500,000` shares. `fundamental-run-today --build-review-list-from-sec` now builds `review_stock_list_<quarter>.csv`, `review_stock_list_rejections_<quarter>.csv`, and `review_stock_list_summary_<quarter>.json` before scoring; daily runs then use that review list. Price checks now use cached parquet data from `/Users/aeternusholdings/.cache/autoresearch_fundamentals` by default; live Yahoo review-list fetch is disabled unless `--allow-live-review-price-fetch` is passed, and the same cache-only source is used for later entry-price checks in this path. Cache-only count for 2026Q2: `10,348` SEC map names -> `1,148` strict SEC-ready -> `1,132` company-data-ready -> `1,115` cached-price names -> `791` final after close/ADV. Summary saved at `eval_results/fundamental/2026-05-13_2026Q2_sec_review_filter_cache_only/cache_only_count_summary.json`. Verification: red tests failed first, then `tests/test_fundamental_review_list_filter.py -q` -> `5 passed`; daily suite `tests/test_fundamental_daily_*.py tests/test_fundamental_review_list_filter.py -q` -> `48 passed, 2 warnings`; touched py_compile passed; CLI help shows new review-list flags. Last updated 2026-05-13T17:40:00-04:00.

**Current note:** User requested simple, understandable English going forward, with no technical jargon unless explicitly needed. Updated persistent communication preference in `memory/MEMORY.md` and the active fundamental history replay plan so future step-by-step explanations start in plain English and define any internal terms first. Last updated 2026-05-13T14:55:00-04:00.
...
**Current note:** Implemented Task 3 for the fundamental consolidated pipeline: `panel.normalize.normalize_complete_panel_rows()` now copies current run rows, normalizes identity/provenance, derives legacy post-LLM subtier flags with `add_post_llm_subtiers()`, optionally fills financial values with companyfacts, and completes rows to `REQUIRED_COMPLETE_PANEL_COLUMNS` with blank flags set to `0`. Added source provenance columns `source_run_root`, `source_artifact`, and `source_artifact_sha256` to the panel schema. TDD red: normalize module import failed before implementation. Verification passed: `tests/test_fundamental_panel_schema.py tests/test_fundamental_panel_financial_values.py tests/test_fundamental_panel_normalize.py -q` -> `11 passed`; `py_compile` passed for touched panel modules. Last updated 2026-05-13T12:24:15-04:00.

**Current note:** Created and QA/QC-reviewed the implementation plan for the consolidated fundamental pipeline at `docs/superpowers/plans/2026-05-13-fundamental-consolidated-pipeline.md`. Goal: one canonical `tradingagents/research/fundamental/src/panel/` producer for complete candidate CSVs and Top10+Plus5+shadow refill outputs, with `Growth/` kept legacy/reference only. Plan includes schema, financial fact extraction, normalization, selector annotation, validation, exporter, standalone `fundamental-build-complete-panel`, daily `fundamental-run-today --emit-complete-panel`, canonical prior-panel validation, docs, and final verification. QA/QC script passed: tasks numbered `1-12`, daily-run emission covered, raw legacy `Growth/` prior panels rejected via `prior_panel_not_canonical`, and QA/QC status marked PASS. Last updated 2026-05-13T11:50:30-04:00.

**Current note:** Filled the `2026Q2` legacy post-LLM subtier columns in the complete fundamental panel. Root cause: current `2026Q2` source final scores do not emit historical `post_llm_tier_1_1_flag` through `post_llm_tier_4_3_flag`; they emit canonical `tier_1_bucket` through `tier_4_bucket` plus post-LLM fields. Filled only `2026Q2` rows using `add_post_llm_subtiers()` formulas; historical rows were left unchanged. Q2 blank counts for all 14 `post_llm_tier_*_flag` columns are now `0`; `110` Q2 rows have at least one positive subtier flag. Validation passed: `23,602` rows, `315` columns, required feature columns missing `0`, duplicate ticker-quarter keys `0`, Q2 Top15/shadow unchanged, output SHA256 `517e17bdc87e8cf4b3160ad964a210bad6b671d95c45e63a843a5b0ab55f5d8d`. Last updated 2026-05-13T11:14:07-04:00.

**Current note:** Filled the non-empty financial attachment gaps in the complete fundamental panel at `outputs/fundamental_backtest/full_complete_panel_2021Q4_2026Q2/fundamental_complete_prellm_to_top15_2021Q4_2026Q2.csv`. The panel now has `23,602` rows and `315` columns after adding source/provenance diagnostics. Raw core blanks went from `revenue 102 / net_income 69 / assets 74 / OCF 108 / ICF 88 / FCF 80` to `revenue 5 / net_income 0 / assets 2 / OCF 0 / ICF 0 / FCF 0`; `symbol`, `cik`, and `company_title` are fully filled. Remaining raw financial blanks are source-unavailable and explicitly tagged in `financial_values_missing_fields`: CIFR revenue for `2021Q4-2022Q4`, SEAT assets `2021Q4`, and SKT assets `2021Q4`. Validation passed: required feature columns missing `0`, duplicate ticker-quarter keys `0`, Q2 Top15/shadow match source artifacts exactly, output SHA256 `64aabc63f9ec40aaf23104e81c6de11b29c7c5b15317c1077c8fbafbcbce831d`. Last updated 2026-05-13T11:06:35-04:00.

**Current note:** Generated the complete fundamental candidate panel requested for `2021Q4` through `2026Q2` at `outputs/fundamental_backtest/full_complete_panel_2021Q4_2026Q2/fundamental_complete_prellm_to_top15_2021Q4_2026Q2.csv`. It has `23,602` ticker-quarter rows and `311` columns: raw historical pre-LLM/LLM/post-LLM detail through `2026Q1`, current `2026Q2` final-score rows from `2026-05-12_2026Q2_hp_rm_final_v3`, required `feature_schema.json` columns, derived stage flags, Top15 annotations, strict shadow refill annotations, and diagnostic return labels. The stale single historical `2026Q2` row was dropped and replaced with `1,299` current Q2 rows. Validation passed: required feature columns missing `0`, duplicate ticker-quarter keys `0`, Q2 Top15 and Q2 shadow lists match source artifacts exactly, output SHA256 `9cb4fbb5764e927d195ed5479d998b166007e7a5ee46683fb5f517ce793d699a`. Last updated 2026-05-13T09:41:01-04:00.

**Current note:** Fixed the RM LLM eligibility timing gap. Daily orchestrator now loads prior-quarter context before Gate 7 and enriches score-ready rows with QoQ fields before `assign_daily_tiers()`, so `repricing_momentum_extension` can be present before LLM eligibility/packet creation. Gate 9 remains the official validation gate for missing/wrong/duplicate prior context. Added regression `test_orchestrator_uses_prior_qoq_before_llm_eligibility_for_rm_only_candidate`: a mid-price weak RM-only candidate with +QoQ repricing and no Tier 1-4 bucket now enters `llm_eligibility.csv`. Updated daily/scoring docs to state prior QoQ context is applied before LLM eligibility. Verification: red test failed before code change, now passes; focused suite `19 passed, 2 warnings`; daily suite `42 passed, 2 warnings`; `py_compile` passed; real 2026-05-12 rerun at `eval_results/fundamental/2026-05-12_2026Q2_hp_rm_final_v3/` passed Gates 1-10 with `209` LLM-complete rows, `0` QoQ missing, Gate 7 RM extension rows `22`, and same Top15 as v2. Last updated 2026-05-13T09:17:15-04:00.

**Current note:** Checked whether RM has an HP-style LLM eligibility gap. Code contract already says `repricing_momentum_extension` requires LLM, and after the HP fix `build_llm_eligibility()` honors that field when it is present. However RM is structurally weaker than HP because Gate 7 assigns LLM eligibility before Gate 9 adds QoQ prior context; without `entry_qoq_pct`, RM buckets do not compute. Proof: `2026-05-12_2026Q2_hp_llm_final_v2/tier_classification.csv` has `0` RM flags at Gate 7, while final scores have `22` `repricing_momentum_extension` rows after QoQ. All 22 yesterday were already Tier 1/3 and LLM-complete, so no practical 2026-05-12 RM miss occurred. Future RM-only/mid-price candidates can still be missed unless QoQ context is loaded before Gate 7. Last updated 2026-05-13T09:08:33-04:00.

**Current note:** Completed the necessary 2026-05-12 HP LLM rerun after fixing daily LLM eligibility. Built corrected HP packets from the audited Q2 run, extracted 4 previously-missing HP candidates (`AAPL`, `LRCX`, `PG`, `SNDK`), merged old `205` + new `4` post-LLM rows into `post_llm_scores_2026Q2_merged_209.csv`, generated Q1 prior context for those 4 from framework diagnostic Q1 rows with Q1 entry moves computed against diagnostic Q4 rows, and reran official Q2 broad final at `eval_results/fundamental/2026-05-12_2026Q2_hp_llm_final_v2/`. Gates `1-10` passed; LLM-complete rows `209`; QoQ missing rows `0`; final reconciliation `1299 + 14 = 1313`. New Top 10 core unchanged: `BBBY, CTOS, NNBR, WEST, XPER, DDD, MAX, TBLA, ANGO, ARRY`. New Plus 5: `UIS, FTRE, CWK, LRCX, CCC`; `LRCX` replaced prior `EVC`. Shadow refill: `0` replacements and same 15 as official. Verification: `tests/test_fundamental_daily_*.py -q` -> `41 passed, 2 warnings`; `py_compile` passed; artifact assertion script passed. Last updated 2026-05-13T08:55:07-04:00.

**Current note:** Implemented the HP LLM eligibility fix. `tradingagents/research/fundamental/src/daily_run/eligibility.py` now uses the shared `features.post_llm_scores.needs_llm()` contract, so Tier 1-4, HP production/research extension, and repricing-momentum extension rows can enter LLM eligibility when coverage/fresh earnings evidence passes. Non-required rows now quarantine as `not_llm_required`. Added failing-then-passing test coverage in `tests/test_fundamental_daily_eligibility_gate.py` for an HP3 high-price production/research candidate with fresh 8-K evidence. Updated daily/scoring docs to remove stale "Tier 1-4 only" wording. Verification: targeted eligibility test failed before fix as expected; `tests/test_fundamental_daily_*.py -q` -> `41 passed, 2 warnings`; `py_compile` passed. Last updated 2026-05-13T08:32:15-04:00.

**Current note:** Verified HP LLM extraction gap in the `2026-05-12` fundamental run. `tier_classification.csv` had `904` HP0 high-price rows and `7` HP3/HP production+research rows (`AAPL, LRCX, LULU, NVDA, PG, ROST, SNDK`), but `llm_eligibility.csv` had `0` HP rows and the packet JSONL contained none of those HP3 names. Root cause: `tradingagents/research/fundamental/src/daily_run/eligibility.py::_has_tier_1_to_4()` only admits `tier_1_bucket` through `tier_4_bucket`; it ignores `hp_production_extension` and `hp_research_extension`, even though `features/post_llm_scores.py::needs_llm()` correctly treats those HP extensions as LLM-required. Result: HP candidates are quarantined as `tier0_or_not_tier_1_to_4`, never sent to LLM, and then remain `pending`/excluded from official Top10+Plus5. Last updated 2026-05-13T08:10:24-04:00.

**Current note:** Audited yesterday's `2026-05-12` broad fundamental Top 10 + Plus 5 + shadow refill artifacts under `eval_results/fundamental/2026-05-12_2026Q2_qoq_final/`. Re-ran the selector in read-only mode from the `205`-row LLM-complete selection input plus `1313`-row SEC coverage manifest; recalculated official Top15 and shadow Top15 matched artifacts exactly. Top 10 core: `BBBY, CTOS, NNBR, WEST, XPER, DDD, MAX, TBLA, ANGO, ARRY`. Plus 5: `UIS, FTRE, CWK, CCC, EVC`. Shadow refill: `0` demoted, `0` replacements, same 15 names as official. Alignment: yes; no hard-gate/formula mismatch found, identity review passed `15/15`, and known broader deterioration-blocking nuance had no effect because blocked/demoted count was `0`. Last updated 2026-05-13T07:42:22-04:00.

**Current note:** Saved the canonical Top 10 + Plus 5 + shadow refill formula reference at `tradingagents/research/fundamental/docs/top10_plus5_shadow_refill_rules.md` and indexed it from `tradingagents/research/fundamental/docs/README.md`. The note captures the exact Top 10 composite/rank formula, Plus 5 exception score and caps, and shadow refill deterioration/refill formulas. Correction applied: intended shadow default blocks actual demoted core tickers from Plus 5 auto-selection, not every deterioration-flagged ticker; HP signal count wording now notes that `HP_SIGNAL_FIELDS` includes `hp_LLM_best`. No code changes were made by this session; active implementation edits from another session were left untouched. Last updated 2026-05-13T07:37:51-04:00.

**Current note:** Checked Situational Awareness LP 13F status live against SEC submissions for CIK `0002045724`. Latest 13F remains accession `0002045724-26-000002`, filed `2026-02-11`, report date `2025-12-31`; no newer Q1 2026 13F is present as of `2026-05-13T06:40:45-04:00`. Local `history_situational_awareness` artifacts already include report dates through `2025-12-31`. Latest Q4 changes include new `BE`, `LITE`, `CIFR`, `KRC`, `WYFI`, `PSIX`, `CLSK`, `BITF`, `LBRT`, `INFY`, `PUMP`; adds/increases include `SNDK`, `CRWV`, `APLD`, `EQT`, `COHR`, `CORZ`, `TSEM`, `SEI`, `HUT`, `IREN`, `RIOT`, `BTDR`; exits include `NVDA`, `VST`, `GLXY`, `AVGO`, `TSM`, `MU`, `STX`. Last updated 2026-05-13T06:40:45-04:00.

**Current note:** Ran non-X dealflow scouts for `2026-05-13` with X-feed excluded from the scout handoff. Technical universe refresh produced `515` unique tickers; technical signal sync processed `516` tickers with `27` buy-zone states; commodity shock found `0` alerts; DoD contract scout found `0` alerts but USASpending returned API `400` for NAICS groups `621` and `237`. The no-X source run patched `load_recent_merged()` to return `{}` in-process, then ran breakout, insider, 13F, technical ignition, FVG, and FMA scouts. Final artifacts: `eval_results/deal_flow/2026-05-13/scout_ticker_summary.json` and `final_dealflow_tickers.json`; `x_manual_feed=0`, `total_unique_tickers=97`, `total_mentions=120`; counts were breakout `46`, 13F `68`, insider `2`, technical ignition `0`, IV `0`, FVG `0`, FMA `4`, commodity `0`, DoD `0`. Handoff verification: no `score`, `rank`, `deal_flow_score`, `core_score`, or `triage_score` fields; metadata keys are only `scouts`. Last updated 2026-05-13T06:18:52-04:00.

**Current note:** Cleaned package-boundary leaks inside production fundamental `src/`. Replaced internal `from src.*` imports with fully-qualified package imports, removed `sys.path` mutation from `src/daily_run/__init__.py` and legacy fundamental CLI path, updated SEC cleanup wrapper template, and made external LLM subprocess set `PYTHONPATH` to repo root plus framework root. Scope: `Growth/` and test shims remain out-of-scope legacy debt. Oracle QA/QC: no blocker. Verification: focused fundamental/SEC/LLM tests `64 passed, 2 warnings`; all `src` py_compile passed; import/CLI smoke passed. Last updated 2026-05-13T08:20:00-04:00.

**Current note:** Fixed daily master-universe input ambiguity. Canonical format is JSON list or JSON object with `items`; compatibility CSV is accepted when it has `ticker`/`symbol` plus `cik`. Malformed/unsupported/empty/schema-invalid master files now hard-stop cleanly at Gate 2 with `invalid_master_universe_format`. CLI help and daily docs now state canonical JSON + CSV compatibility. Oracle QA/QC verdict: no blocker, ship. Verification: targeted universe/orchestrator/CLI tests `17 passed, 2 warnings`; daily suite `43 passed, 2 warnings`; touched py_compile passed. Last updated 2026-05-13T08:00:00-04:00.

**Current note:** Added the clean fundamental operator review folder. Successful broad-final daily publish now mirrors only review-facing artifacts into `output_root/final/`: stable `fundamental_final_scores.csv`, `high_conviction_top15.csv/json`, `high_conviction_top15_daily_recommendation.md`, and `publish_guard_summary.json`, while preserving root artifacts for compatibility. Oracle QA/QC found no blocker; future hardening can make missing Top15 artifact paths a hard stop. Verification: daily orchestrator+CLI tests `11 passed, 2 warnings`; daily suite `37 passed, 2 warnings`; touched orchestrator py_compile passed. Last updated 2026-05-13T07:45:00-04:00.

**Current note:** Patched the second high-priority fundamental architecture gap: SEC pipeline config/naming is now quarter-window driven. Added `SecPipelineConfig`; coverage/download/validation/incremental/delta modules derive manifest paths from configured quarters while preserving default `2021Q4_2026Q1`; daily coverage passes `quarters=[quarter]` instead of mutating globals. Oracle QA/QC approved after fixing stale `18 coverage rows` policy text. Verification: SEC focused tests `18 passed`; daily+SEC focused tests `51 passed, 2 warnings`; touched py_compile passed. Last updated 2026-05-13T07:30:00-04:00.

**Current note:** Patched the first high-priority fundamental architecture gap: official `fundamental-run-today` now requires prior-quarter comparison data via `--prior-final-scores` before broad-final Top10 + Plus5 publish. Gate 9 now loads/validates prior context, rejects missing/wrong-quarter/duplicate prior ticker-quarter rows, enriches current rows with `entry_qoq_pct`, `score_change`, and `prior_pre_llm_fundamental_score`, passes prior rows into final scoring, and hard-stops if any LLM-complete final row lacks QoQ fields. Publish reconciliation now uses final rows + explicit quarantines = broad universe. Verification: focused daily suite `37 passed, 2 warnings`; broad fundamental-focused suite `196 passed, 2 warnings`; touched py_compile passed. Oracle final QA/QC approved. Last updated 2026-05-13T07:05:00-04:00.

**Current note:** Fundamental framework architecture consolidation is in progress. Canonical framework home is now `tradingagents/research/fundamental/` with domain-owned CLI implementation in `tradingagents/research/fundamental/src/cli/commands.py`, root `/cli/commands/fundamental.py` as compatibility wrapper, canonical docs moved under `tradingagents/research/fundamental/docs/`, new review docs `FRAMEWORK_INDEX.md`, `ARCHITECTURE.md`, and `ARTIFACTS.md`, and canonical generated-run defaults under `tradingagents/research/fundamental/runs/`. Removed stale hardcoded SEC/dealflow/backtest default paths in favor of framework/cache config. Verification so far: broad fundamental-focused suite `191 passed, 2 warnings`; py_compile passed. Last updated 2026-05-12T17:20:00-04:00.

**Current note:** 2026-05-12 broad fundamental Top 10 + Plus 5 is publish-ready after QoQ merge and identity/staleness guard. Combined Q2 main LLM (`205`) with Q1 comparison LLM (`227`); Q2 LLM-complete rows have QoQ fields present `205/205`. Final artifact root: `eval_results/fundamental/2026-05-12_2026Q2_qoq_final/`. Top 10 core: `BBBY, CTOS, NNBR, WEST, XPER, DDD, MAX, TBLA, ANGO, ARRY`. Plus 5: `UIS, FTRE, CWK, CCC, EVC`. Shadow refill replacements: `0`. Identity review: `15/15` pass, `0` quarantine, no refill needed. BBBY note: active current NYSE issuer CIK `1130713`, not legacy bankrupt BBBYQ. Publish guard reconciliation: `1299` score-ready + `14` score-input quarantine = `1313`. Last updated 2026-05-12T16:35:00-04:00.

**Current note:** Ran non-X dealflow for `2026-05-12` with X-feed excluded from the scout handoff. Technical universe refresh produced `515` unique tickers; technical signal sync processed `516` tickers with `23` buy-zone states; commodity shock found `0` alerts; DoD contract scout found `0` alerts but USASpending returned API `400` for NAICS groups `621` and `237`. The no-X source run patched `load_recent_merged()` to return `{}` in-process, then ran breakout, insider, 13F, technical ignition, FVG, and FMA scouts. Final artifacts: `eval_results/deal_flow/2026-05-12/scout_ticker_summary.json` and `final_dealflow_tickers.json`; `x_manual_feed=0`, `total_unique_tickers=97`, `total_mentions=125`; counts were breakout `46`, 13F `68`, insider `2`, technical ignition `0`, IV `0`, FVG `5`, FMA `4`, commodity `0`, DoD `0`. Handoff verification: no `score`, `rank`, `deal_flow_score`, `core_score`, or `triage_score` fields; metadata keys are only `scouts`. Last updated 2026-05-12T10:01:49-04:00.
...
**Current note:** Documented the required 10-gate automation contract for the request "run the fundamental framework for today" in `tradingagents/research/fundamental/docs/daily_run_gate_sequence.md`. The gates are: run identity/snapshot, universe construction/drift, filing/companyfacts coverage, fetch/materialization, pre-LLM readiness, trade-date/entry-price, Tier 0-4/LLM eligibility, LLM packet/extraction validation, final scoring + HP/RM, and Top10 + Plus5 + shadow publish. Also linked it from `tradingagents/research/fundamental/docs/daily_universe_llm_funnel_contract.md`. Last updated 2026-05-12T06:23:04-04:00.
...
**Current note:** Right-Tail Scout/Demote Review implementation plan is drafted/revised at `docs/superpowers/plans/2026-05-09-right-tail-scout-demote-review.md`. Latest revision incorporates user review: `top15_exception_candidate_queue` terminology, supplier/bottleneck +10, repricing momentum priority/extension scoring, overrideable-only soft demote candidate routing, severity-forces-demote-flag schema rule, optional daily target audit, `--top15-selected-csv`, visibility/actionable/buy routing metrics, Top15 selected hash guard, shared `signal_utils.py`, schema-safe supplier keyword matching, and clarified selected-status vs queue-type semantics. Final blocker review approved. No implementation yet.

**Current note:** Fundamental Top15 exception sleeve backtest is implemented and committed as `bce7c6d7818f724064bd54725e8eab0cf0aaf78b`. New module `tradingagents/research/fundamental/backtests/high_conviction_top15_exception_sleeve.py` compares prior Top10 baseline against three Top15 exception-sleeve variants, writes selected names, strategy summaries, core-vs-exception contribution, diagnostics, right-tail/left-tail comparisons, missed right-tail, manifest, and README under `outputs/fundamental_backtest/high_conviction_top15_exception_sleeve/`. Generated committed outputs: selected `399`, strategy_by_quarter `72`, diagnostics `54`, right_tail_capture `92`, missed_right_tail `85`; input eligible rows `21182`, quarters `18`. Verification: `/Library/Frameworks/Python.framework/Versions/3.14/bin/python3 -m pytest tests/test_high_conviction_top15_backtest.py tests/test_high_conviction_top15_exception_sleeve.py -q` -> `22 passed`; module run completed.

**Current note:** Fundamental Top15 exception sleeve is implemented and committed as `9991398c20ad3d1461154c7b1c24c2843b3cb26d`. New selector `high_conviction_top15_v3_exception_sleeve` preserves existing Top10 core behavior and adds up to five right-tail exception research/starter-underwriting names with RM/HP/theme/AKG eligibility, deterministic scoring, slot controls, duplicate-core prevention, and Top15-specific recommendation wording. New CLI `fundamental-top15` writes `high_conviction_top15.csv/json/high_conviction_top15_daily_recommendation.md`; existing `fundamental-top10` output names remain unchanged. Verification: `/Library/Frameworks/Python.framework/Versions/3.14/bin/python3 -m pytest tests/test_fundamental_high_conviction_top10.py tests/test_cli_fundamental_top10.py tests/test_high_conviction_top15_exception_sleeve.py -q` -> `36 passed, 2 warnings`.

**Current note:** Fundamental Top15 exception sleeve is implemented through analysis. New selector/CLI supports `high_conviction_top15_v3_exception_sleeve` via `fundamental-top15`, writing `high_conviction_top15.csv/json/high_conviction_top15_daily_recommendation.md` without changing Top10 outputs. Backtest module: `tradingagents/research/fundamental/backtests/high_conviction_top15_exception_sleeve.py`; generated bundle: `outputs/fundamental_backtest/high_conviction_top15_exception_sleeve/`. Analysis script/report: `scripts/analyze_fundamental_top15_exception_sleeve.py`, `tradingagents/research/fundamental/docs/top15_exception_sleeve_observed_backtest_analysis.md`; companion tables: `outputs/fundamental_backtest/analysis_top15_exception/`. Main result: Top15 v3 `avg_return_90d=17.28%`, `+30 hit=28.89%`, `-30 loser=7.22%`, vs Top10 v2 `15.50%`, `26.85%`, `7.22%`; exception sleeve had `18` picks, `35.39%` avg 90d, `50.00%` +30 hit, `5.56%` -30 loser. Caveat: Top15 remains research/starter-underwriting, not 15 equal-weight buys; captured `AAOI` and `CRDO` from named target set but still missed most low-score RM/theme-wave examples. Verification: focused pytest `51 passed, 2 warnings`; backtest/analysis manifest hash validation `0` failures. Final review status: not approved pending spec reconciliation; blockers are standalone `market_repricing_score >= 10` eligibility missing vs plan and AAOI target-capture reporting inconsistency between bundle manifest and analysis table/doc.

**Current note:** Fundamental Top10 observed-data analysis and daily recommendation output are implemented. Main report: `tradingagents/research/fundamental/docs/top10_observed_backtest_analysis.md`. Generator: `scripts/analyze_fundamental_top10_observed_backtest.py`. Companion tables live under `outputs/fundamental_backtest/analysis/`. Daily selector `select_from_csv()` now writes `high_conviction_top10_daily_recommendation.md` beside `high_conviction_top10.csv/json`; CLI prints the path. Daily recommendation uses `high_conviction_top10_v2_final` as the observed-data v2 operating setting, max positions `10`, single-RM-signal bucket priority, RM2+/HP caution, manual macro permission, and AKG theme/T5 forward-validation caveat. Runbook `docs/research/aeternus-daily-pipeline-debug-runbook.md` now documents the post-finalized-dealflow/fundamental-scores handoff command. QA: `qa_leakage_verification.json` has `passed=true`; no outcome/monitoring/final-rank selection overlap; AKG/macro missing-field available counts all `0`; analysis manifest hash validation `0` failures. Verification: `tests/test_fundamental_high_conviction_top10.py tests/test_cli_fundamental_top10.py -q` -> `24 passed, 2 warnings`; temp live CLI check wrote CSV/JSON/recommendation MD; final reviewer approved.
...
- Daily X-feed Grok automation ran for `2026-05-17`:
  ...
  - pass `16`, `Blindspot & Unmapped Ticker Audit`, added `NOK`, `AAOI`, `PENG`
...
- Daily X-feed Grok automation ran for `2026-05-14`:
  ...
  - edge types: `account_mentions_ticker`, `ticker_linked_to_theme`, `ticker_co_mentioned`
  - low-yield valid passes: `5`, `6`, `9`, `14`, `15`
  - pass `16`, `Blindspot & Unmapped Ticker Audit`, added `NOK`, `P`, `MP`
  - operational note: run completed without recovery extraction; direct DOM prompt set + submit-button click worked, Grok Copy button not used

- Daily X-feed Grok automation ran for `2026-05-13`:
  - completed passes `1-16`
  - target model used: `Grok 4.3 (beta)`
  - final readiness: `ready=True`, finalized manifest exists
  - raw archives: 16
  ...
  - edge types: `account_mentions_ticker`, `ticker_linked_to_theme`, `ticker_co_mentioned`
  - low-yield valid passes: `3`, `5`, `6`, `8`, `9`, `10`, `14`, `15`, `16`
  - pass `16`, `Blindspot & Unmapped Ticker Audit`, completed with valid zero-ticker JSON
  - operational note: pass `1` timed out in runner but was recovered from completed Grok page text after explicitly selecting the latest Grok tab; future runs should keep rebinding Chrome to the Grok tab before extraction/submission
...
- Daily X-feed Grok automation ran for `2026-05-09`:
  ...
  - edge types: `account_mentions_ticker`, `ticker_linked_to_theme`, `ticker_co_mentioned`
  - low-yield valid passes: `5`, `7`, `13`, `15`
  - operational note: direct DOM set + input event remained required; clipboard/Copy button not used
  - operational note: pass 11 timed out once and succeeded on rerun; passes 14 and 15 timed out but had valid completed Grok JSON and were recovered by explicitly selecting the latest Grok tab before page-text extraction
...
- Added V3/S7 conflict rule: QQQ V3 overnight long is suppressed when `QQQ < QQQ_SMA200` and SPY S7a/b is active. `IndexOverlayEngine.get_signal("QQQ")` exposes raw/suppressed overnight metadata; portfolio plans remove V3 residual index order and park residual cash when hedge mode is `S7_HEDGE`. Verification: targeted phase/index/paper/portfolio/momentum/hedge/market tests `124 passed, 2 warnings`; py_compile passed; live momentum-scan shows current suppression false.
- Built `aeternus qqq-daily --format table|json`: single QQQ daily report with V3 RTH/overnight, QQQ-gated SPY S7 hedge, CCWyckoff S2/S3/S4/S7b CC-day bucket, and as-of-truncated V3/CC return buckets. Oracle QAQC initially blocked on as-of leakage/alignment; fixed by sorting/asserting aligned dates and truncating return stats to selected as-of. Final Oracle status: Approved. Verification: `42 passed, 2 warnings`; py_compile passed; live smoke current as-of `2026-05-12` action = no RTH long, overnight long, hedge off, no CC day.
...
## Current fundamental SEC review-list flow

- Review-list flow now matches the intended plain flow: check SEC and Yahoo cache first, fetch missing SEC/Yahoo data second, then decide kept/rejected/pending.
- Yahoo is only fetched for names that already have required SEC evidence and usable company data; it is not fetched for the full SEC ticker map.
- Live Yahoo rows are cached to `review_price_cache_<quarter>_<as_of>.parquet` in the run folder and the shared market cache folder.
- Missing local cache is no longer treated as an automatic stock failure; it becomes pending/fetch-needed until the fetch path has a chance to run.
- Full cache-only test run: `eval_results/fundamental/2026-05-13_2026Q2_review_list_cache_test_v2`.
...
- User stopped `2025Q3` walkforward LLM work to prioritize current-quarter `2026Q2`. Do not combine or publish partial `2025Q3` chunks unless user explicitly resumes that quarter.
- Confirmed no active `2025Q3` LLM/fundamental processes after interruption.
- SEC storage fix: `complete_submission` fetch now extracts requested documents without storing full complete-submission wrapper `.txt` files.
- Redundant `live_sec/complete_submissions` wrapper files were deleted; extracted documents, companyfacts, submissions, LLM cache, scores, and PIT were not deleted. Free space increased by about `43 GB`.
- Current-focus run root: `tradingagents/research/fundamental/runs/2026-05-15/2026Q2/current_focus_v1`.
- Prior QoQ input: `eval_results/fundamental/2026-05-12_2026Q1_tier14_readiness/fundamental_final_scores_2026-05-12.csv`.
- `2026Q2` counts: universe `1,374`; final rows `1,316`; LLM packets `537`; current LLM-complete rows `270`; prior recovery rows `267`; QoQ missing rows `0`; Top15 rows `15`; shadow rows `15`; shadow refill replacements `0`.
- Prior-impossible/no-filing names: `DRVN`, `KOS`, `TRAW`.
- Verification: complete panel passed; focused daily/coverage/LLM tests `62 passed`; `git diff --check` passed.
