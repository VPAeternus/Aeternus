...
**Current note:** Paper execution god-file split has started with an Oracle-approved micro-slice. `tradingagents/execution/` now contains `constants.py`, `modes.py`, `json_io.py`, and `alpaca_client.py`; `tradingagents/graph/paper_execution.py` keeps legacy aliases and monkeypatch seams. Verification passed: `py_compile` plus `pytest tests/test_paper_execution.py tests/test_portfolio_construction.py tests/test_multi_account.py tests/test_cli_commands_import.py -q` → `87 passed, 1 skipped`. Known stale/pre-existing `tests/test_exit_enforcement.py` failures around thesis invalidation/conviction decay are outside this refactor slice.
