# Walkforward Stock Screener Framework

This file is the source of truth for the walkforward stock screener and picker. Update this file first when the framework changes.
...
Use walkforward ranking to choose between tickers such as `BE`, `NVDA`, `AAOI`, `MU`, `LITE`, `GOOGL`, and `SNDK`.
...
- `Growth/walkforward_event_picker.py`
...
- `Growth/earnings_8k_sec_parser/walkforward_candidates.csv`
- `Growth/earnings_8k_sec_parser/walkforward_picks.csv`
- `Growth/earnings_8k_sec_parser/walkforward_summary.csv`

Framework doc:

- `Growth/walkforward_stock_screener_framework.md`
...
- AKG current universe is not point-in-time for historical walkforward unless we snapshot/version it by decision date.
...
- Yahoo current market cap is not point-in-time and should not be used for historical walkforward filters without caveat.
...
- For walkforward quarters, XBRL facts must be selected with each ticker's earnings event filing date as `asof_date`. Do not use future quarter-end filings if they were not filed by the event date.
- Score components: profitability, operating cash flow margin, FCF proxy margin, financing dependence, asset efficiency.
- Current combined walkforward file through 2022: `Growth/earnings_8k_sec_parser/pre_llm_fundamental_score_2021Q4_2022Q4.csv`.
