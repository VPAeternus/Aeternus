...
def write_weekly_reports(
    ...
    *,
    ...
    new_entry = [
        ...
        if clean(row.get("candidate_state")) in {"new_signal", "research_queue", "watchlist", "llm_pending"}
    ...
    return [
        write_csv(out_dir / f"{stamp}_new_entry_research_queue.csv", sorted(new_entry, key=lambda r: to_float(r.get("entry_score_0_100")) or 0, reverse=True)),
