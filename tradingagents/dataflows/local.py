...
def _safe_date_from_epoch(epoch_value) -> str:
    try:
        ts = int(epoch_value or 0)
...
def get_finnhub_news(
    ...
    # Prefer live Finnhub API when FINNHUB_API_KEY is available.
    try:
        live_payload = _finnhub_get(
            "/company-news",
            {"symbol": query.upper(), "from": start_date, "to": end_date},
        )
    except Exception:
        live_payload = None

    if isinstance(live_payload, list) and len(live_payload) > 0:
        combined_result = ""
        for entry in live_payload:
            headline = str(entry.get("headline", "")).strip()
            summary = str(entry.get("summary", "")).strip()
            if not headline and not summary:
                continue
            day = _safe_date_from_epoch(entry.get("datetime")) or "unknown-date"
            combined_result += f"### {headline} ({day})\n{summary}\n\n"
        if combined_result:
            return f"## {query} News, from {start_date} to {end_date}:\n{combined_result}"

    # Fall back to local cached datasets.
    try:
...
def get_finnhub_company_insider_sentiment(
    ...
    try:
        live_payload = _finnhub_get(
            ...
            {"symbol": ticker.upper(), "from": before, "to": curr_date},
        )
    except Exception:
        live_payload = None

    if isinstance(live_payload, dict):
        sentiment_data = live_payload.get("data", [])
        if sentiment_data:
            result_str = ""
            seen = set()
...
def get_finnhub_company_insider_transactions(
    ...
    if isinstance(live_payload, dict):
        tx_data = live_payload.get("data", [])
    elif isinstance(live_payload, list):
        tx_data = live_payload
    else:
        tx_data = []
    ...
    if tx_data:
        ...
        for entry in tx_data:
            ...
            tx_price = entry.get("transactionPrice", "")
            tx_code = entry.get("transactionCode", "")
            key = (filing_date, name, change, share, tx_price, tx_code)
            if key in seen:
                continue
            seen.add(key)
            result_str += (
