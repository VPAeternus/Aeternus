...
class DissentRecord(BaseModel):
    """Captures significant disagreement between debate participants."""
    dissenter_role: str
    dissent_topic: str
...
class RiskVerdict(BaseModel):
    ...
    invalidation_conditions: List[InvalidationCondition] = Field(default_factory=list)
    drawdown_mode: bool = Field(default=False, description="True if portfolio is in drawdown — defensive posture")
    re_entry_conditions: List[str] = Field(default_factory=list, description="Conditions for re-entry if in drawdown mode")
